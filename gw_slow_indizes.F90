...
module computation_functions
  ...
  implicit none
  private
  public :: compute_Giw, compute_Gconv, compute_P, Giw2Gtau, compute_W, compute_SE, compute_n, compute_V
...
! output - should go into io.f90
  if(myid.eq.master) then
    call local_output_Matsub(Giw,1,trim(outfolder)//"Gw.dat") ! this routine is in io.f90
    call local_output_Matsub_diagonal(Giw,1,trim(outfolder)//"Gw_diag.dat") ! this routine is in io.f90

! output - should go into io.f90
    open(unit=10,file=trim(outfolder)//"Gkw.dat")
      write(10,*) "## ikp, iw, iomega, RE[G_ii(ikp,iw)], IM(G_ii(ikp,iw)]"
!      write(10,*) "## ij in this order: 1_1, 1_2, .., 1_ndim, 2_1, 2_2, .., 2_ndim, .., ndim_ndim"
      write(10,*) "##"
      do ikp=1,nkp
      do iw=1,min(250,nw)
  ...
  open(unit=11,file=trim(outfolder)//"Hk.dat")
...
! parallelization - communication
  ...
  if(myid.eq.master) then
    write(*,*) 'output of Ploc(iv) to file Pv_MO.dat'
    call local_output_Compound(P,0,trim(outfolder)//"Pv.dat")
  endif
...
subroutine Giw2Gtau(nw,L,beta,Giw,Gtau)
! input/output
  integer, intent(in) :: nw,L
...
! - offdiagonal elements are continous (no jump)
  ...
  do ina=1,ndim
     mat(ina,ina)=1.d0
  enddo
  call local_output_tau(Gtau,ndim,L,beta,1,mat,trim(outfolder)//"Gt.dat")
  deallocate(mat)
...
! parallelization - communication
  ...
  if(myid.eq.master) then
    ...
    call local_output_Compound(W,0,trim(outfolder)//"Wv.dat")
  endif
...
! input/output
  complex(kind=8), intent(in) :: Giw(ndim,ndim,nkp,2*nw)
  complex(kind=8), intent(in) :: Vend(ndim**2,ndim**2,nkp)!,V(ndim**2,ndim**2,nkp,nw)
...
! auxiliaries
  ...
  complex(kind=8) :: tmpW(ndim,ndim,ndim,ndim,nkp,nw)!, tmpV(ndim,ndim,ndim,ndim,nkp,nw)
...
!    tmpV(inverse_l(i),inverse_r(i),inverse_r(j),inverse_l(j),ikq,iv) = V(i,j,imq(ikq),iv) 
  enddo
  enddo
  enddo
  enddo
...
!! \Sigma_kl{k,iwn}
  ...
  if (myid.eq.master) then
    call local_output_Matsub(FT,1,trim(outfolder)//"Sw_FT_MO.dat") ! this routine is in io.f90
  endif
...
!##########################################
    ...
    do ikp=ikstart,ikend ! 1,nkp
      ...
      do j=1,ndim
        ...
        do iw=1,nw
          ...
          ST(k,l,ikp,:) = ST(k,l,ikp,:) + tmpVend(k,i,j,l,ikq) & 
  ...
  do ikp=ikstart,ikend
    ...
    do l=1,ndim 
      do i=1,ndim
        do ikq=1,nkp
          ST(k,l,ikp,:) = ST(k,l,ikp,:) + tmpVend(k,i,i,l,ikq) * wtkp(ikq) / 4.d0
        enddo !ikq
      enddo !i
    enddo !l
    enddo! k
  ...
  if(myid.eq.master) then
    call local_output_Matsub(ST,1,trim(outfolder)//"Sw_ST_MO.dat") ! this routine is in io.f90
  endif
 ...
 allocate(mpi_cwork(nkp))
  ...
  if(myid.eq.master) then
    ...
    call local_output_Matsub(SE,1,trim(outfolder)//"Sw.dat") ! this routine is in io.f90
    call local_output_Matsub_diagonal(SE,1,trim(outfolder)//"Sw_diag.dat") ! this routine is in io.f90

    open(unit=10,file=trim(outfolder)//"Skw.dat")
...
! compare signs and choose mu_mid as new mu_min or mu_max
     if( (real(ntot-nmin) .gt. 0.d0) .and. (real(ntot-nmax) .lt. 0.d0) ) then ! ncur is increasing with mu
        if (abs(real(ntot-nmid)) .le. 1d-9 ) then
          ...
          if(myid.eq.master) write(*,*) "Bisection yielded n = ", n
          exit
...
if (cyc .eq. 1) then ! this describes a oneshot with updated G afterwards
...
! Difference between old and new Self-Energy - self consistency
  ...
  if (cyc .ge. 2) then    ! TESTING PURPOSES - FIXED AMOUNT OF CYCLES - flagSE must be F - exit after 2 cycles
